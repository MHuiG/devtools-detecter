...
方案使用基准的执行时间作为下面的性能采样的临界水平($CriticalLevel$)。
...
理论上是可行的，实验中发现 `console hack` 性能采样函数在 IE 控制台打开和关闭具有不同的表现，只是差异的倍数较小。导致很难寻找一个基准函数 `Benchmark()` 或者动态调整临界水平的方法去 hack 所有浏览器。但是笔者坚信这样的基准函数或者动态调整临界水平的方法是存在的。
